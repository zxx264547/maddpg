...
from MADDPG import ValueNetwork, PolicyNetwork, DDPG, ReplayBuffer, ReplayBuffer, PolicyNetwork


use_cuda = torch.cuda.is_available()
device = torch.device("cuda" if use_cuda else "cpu")

parser = argparse.ArgumentParser(description='Single Phase MADDPG')
parser.add_argument('--env_name', default="123bus",
                    help='name of the environment to run')
parser.add_argument('--algorithm', default='ddpg', help='name of algorithm')
...
if args.env_name == '123bus':
    ...
    num_pv_agent = env.pv_agentnum
    num_es_agent = env.es_agentnum
...
obs_dim = env.obs_dim
pv_action_dim = env.pv_action_dim
es_action_dim = env.es_action_dim
hidden_dim = 100
if ph_num == 3:
...
pv_agent_list = []
es_agent_list = []
pv_replay_buffer_list = []
es_replay_buffer_list = []

"""
创建光伏智能体
"""
for i in range(num_pv_agent):
    # if ph_num == 3:
    ...
    if args.algorithm == 'maddpg' and not ph_num == 3:
        policy_net = PolicyNetwork(env=env, obs_dim=obs_dim, action_dim=pv_action_dim, hidden_dim=hidden_dim).to(
            device)
        target_policy_net = PolicyNetwork(env=env, obs_dim=obs_dim, action_dim=pv_action_dim,
    ...
    #         device)
    value_net = ValueNetwork(obs_dim=obs_dim, action_dim=pv_action_dim, hidden_dim=hidden_dim).to(device)
    target_value_net = ValueNetwork(obs_dim=obs_dim, action_dim=pv_action_dim, hidden_dim=hidden_dim).to(device)
    ...
    pv_replay_buffer = ReplayBuffer(capacity=1000000)

    pv_agent_list.append(agent)
    pv_replay_buffer_list.append(pv_replay_buffer)
    ...
    for i in range(num_pv_agent):
        ...
        # value_net = ValueNetwork(obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim).to(device)
        if args.algorithm == 'maddpg' and not ph_num == 3:
            policy_net = PolicyNetwork(env=env, obs_dim=obs_dim, action_dim=es_action_dim, hidden_dim=hidden_dim).to(
                device)
            target_policy_net = PolicyNetwork(env=env, obs_dim=obs_dim, action_dim=es_action_dim,
                                              hidden_dim=hidden_dim).to(device)
        # elif args.algorithm == 'safe-ddpg' and ph_num == 3 and args.safe_type == 'three_single':
        #     policy_net = SafePolicy3phase(env, obs_dim, action_dim, hidden_dim, env.injection_bus_str[i]).to(device)
        #     target_policy_net = SafePolicy3phase(env, obs_dim, action_dim, hidden_dim, env.injection_bus_str[i]).to(device)
        ...
        #     target_policy_net = PolicyNetwork(env=env, obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim).to(
        #         device)
        value_net = ValueNetwork(obs_dim=obs_dim, action_dim=es_action_dim, hidden_dim=hidden_dim).to(device)
        target_value_net = ValueNetwork(obs_dim=obs_dim, action_dim=es_action_dim, hidden_dim=hidden_dim).to(device)
        ...
        agent = DDPG(policy_net=policy_net, value_net=value_net,
                     target_policy_net=target_policy_net, target_value_net=target_value_net, value_lr=vlr,
                     policy_lr=plr)

        es_replay_buffer = ReplayBuffer(capacity=1000000)

        es_agent_list.append(agent)
        es_replay_buffer_list.append(es_replay_buffer)
...
if (FLAG == 0):
    # load trained policy
    for i in range(num_pv_agent):
        if ph_num == 3 and args.algorithm == 'maddpg':
        ...
        pv_agent_list[i].policy_net.load_state_dict(policynet_dict)

    for i in range(num_es_agent):
        if ph_num == 3 and args.algorithm == 'maddpg':
            policynet_dict = torch.load(
                f'checkpoints/{type_name}/{args.env_name}/{args.algorithm}/{args.safe_type}/policy_net_checkpoint_a{i}.pth')
        else:
            policynet_dict = torch.load(
                f'checkpoints/{type_name}/{args.env_name}/{args.algorithm}/policy_net_checkpoint_a{i}.pth')
        es_agent_list[i].policy_net.load_state_dict(policynet_dict)
    ...
    for episode in range(num_episodes):
        ...
        episode_reward = 0
        # todo:last_action到底是啥
        # last_action = np.zeros((num_pv_agent, ph_num))  # if single phase, 1, else ,3
        ...
        for step in range(num_steps):
            ...
            for i in range(num_pv_agent):
                ...
                # 获取当前智能体的动作，并加上一个噪声，随着episode的增加而减小，并将动作控制在阈值之内
                action_agent = (pv_agent_list[i].policy_net.get_action(np.asarray([state[i]]))
