...
class DDPG:
    ...
    # 说明：从回放缓冲区中取样，计算损失并更新策略和价值网络的参数。
    ...
    def train_step_3ph(self, replay_buffer, batch_size,
        ...
        policy_loss = self.value_net(state, last_action - self.policy_net(state))
        policy_loss = (-policy_loss.mean() + torch.norm(self.policy_net(torch.ones_like(state)), 2)
                       + 0.05 * torch.exp(jacob_ii) + 0.05 * torch.exp(jacob_dif))
...
class SafePolicy3phase(nn.Module):
    def __init__(self, env, obs_dim, action_dim, hidden_dim, bus_id, scale=0.15, init_w=3e-3):
        ...
        for phase in env.injection_bus[bus_id]:
            if phase == 'a':
                self.policy_a = SafePolicyNetwork(env, obs_dim, action_dim_per_phase, hidden_dim, scale=0.15,
                                                  init_w=3e-3)
            if phase == 'b':
                self.policy_b = SafePolicyNetwork(env, obs_dim, action_dim_per_phase, hidden_dim, scale=0.15,
                                                  init_w=3e-3)
            if phase == 'c':
                self.policy_c = SafePolicyNetwork(env, obs_dim, action_dim_per_phase, hidden_dim, scale=0.15,
                                                  init_w=3e-3)
    ...
    def forward(self, state):
        ...
        action += (torch.maximum(state - 1.03, torch.zeros_like(state).to(self.device)) - torch.maximum(0.97 - state,
                                                                                                        torch.zeros_like(
                                                                                                            state).to(
                                                                                                            self.device))) * 0.01
        return action
...
class PolicyNetwork(nn.Module):
    def __init__(self, env, obs_dim, action_dim, hidden_dim, init_w=3e-3):
        ...
        self.env = env
        # self.linear1 接收观察值的维度 obs_dim 并输出隐藏层的维度 hidden_dim。
        self.linear1 = nn.Linear(obs_dim, hidden_dim)
        # self.linear2 接收隐藏层的维度 hidden_dim 并输出另一个隐藏层的维度 hidden_dim。
        self.linear2 = nn.Linear(hidden_dim, hidden_dim)
        # self.linear3 接收隐藏层的维度 hidden_dim 并输出动作的维度 action_dim。
        self.linear3 = nn.Linear(hidden_dim, action_dim)

        # 初始化最后一层的权重和偏置，在区间 [-init_w, init_w] 上均匀分布。
...
class LinearPolicy(nn.Module):
    ...
    def forward(self, state):
        state.requires_grad = True
        x = (torch.maximum(state - 1.03, torch.zeros_like(state).to(self.device)) - torch.maximum(0.97 - state,
                                                                                                  torch.zeros_like(
                                                                                                      state).to(
                                                                                                      self.device))) * torch.square(
            self.slope)
        return x
